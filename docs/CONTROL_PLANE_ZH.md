...
如果要按 job type 做 worker ownership / 分片，可以限制内置 worker 或独立 worker 只处理指定任务：
...
如果希望通过 job type 明确 worker ownership / 分片范围，可以设置 `AGENT_ARCHITECT_LAB_CONTROL_PLANE_WORKER_ALLOWED_JOB_TYPES`。
...
- `/health`、`/metrics`、`/workers` 以及独立 worker CLI 输出现在都会暴露 `allowed_job_types`，方便运维确认 worker ownership 和分片配置
...
查看生产环境 deploy policy 和某个 release 的 deploy readiness：
